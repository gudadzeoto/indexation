...
// POST /api/report2
// Inserts one row per work period into [indexation].[dbo].[report_2]
  ...
  try {
    ...
    const nextPrintNumResult = await pool
      ...
      .query(`
        ...
        FROM [${process.env.DB_NAME_INDEXATION}].[dbo].[report_2] WITH (UPDLOCK, HOLDLOCK)
      `);
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      await request.query(`
        INSERT INTO [${process.env.DB_NAME_INDEXATION}].[dbo].[report_2]
