...
// POST /api/report1
// Inserts one row per work period into [indexation].[dbo].[report_1]
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      request.input(
        "money",
        sql.Decimal(18, 2),
        row.money !== null && row.money !== undefined && row.money !== ""
          ? Number(row.money)
          : 0,
      );
      request.input(
      ...
      request.input(
        "pn",
        sql.Decimal(18, 3),
        row.pn !== null && row.pn !== undefined && row.pn !== ""
          ? Number(row.pn)
          : null,
      );
      request.input(
      ...
      await request.query(`
        INSERT INTO [${process.env.DB_NAME_INDEXATION}].[dbo].[report_1]
          ([printnum], [contractnum], [customer], [year], [selectmonth2], [selectnumber2],
           [costofwork], [money], [index], [pn], [finishInput], [Legal_Code], [Full_Name], [Abbreviation])
        VALUES
          (@printnum, @contractnum, @customer, @year, @selectmonth2, @selectnumber2,
           @costofwork, @money, @index, @pn, @finishInput, @Legal_Code, @Full_Name, @Abbreviation)
      `);
